...
def ExecuteWithLogs(func_desc, log_file, func_to_measure):   
...
with open('log.txt', 'a') as log_file:
    ...
    ExecuteWithLogs("Training", log_file, lambda _ = None: model.fit_generator(trainGenerator, SAMPLE_COUNT, EPOCH_COUNT))   

    #Executing testing with timestamps and measurements
    testGenerator = CreateTestGenerator(TEST_PATH, TARGET_SIZE, MAX_VALUE)
    result = ExecuteWithLogs("Testing", log_file, lambda _ = None: (model.predict_generator(testGenerator, STEPS, verbose = 1)))

    #Executing result saving with timestamps and measurements
    ExecuteWithLogs("Saving results", log_file, lambda _ = None: 
